/*-------- Get Bonus toggler button -------*/
document.getElementById("get-bonus-btn").addEventListener("click", function () {
    //change btn click style
    toggleStyleBtn("get-bonus-btn");

    //show transfer money form section
    showFormSection("get-bonus-form");
});


/*------- Get Bonus form button ---------*/
document.getElementById("getBonus-btn").addEventListener("click", function (event) {
    event.preventDefault();

    //get the coupon code
    const couponCode = document.getElementById("getBonus-input").value;

    //validation [temporary]
    if (couponCode === "payoo100") {
        ...
        document.getElementById("getBonus-input").value = "";

        // show it in transaction section
        let date = new Date();
        let now = date.toLocaleString("en-US", {hour: 'numeric', minute: 'numeric', hour12: 'true'});
        ...
        div.innerHTML = `
            <div class="px-4 py-3 bg-white border border-[#0808081A] rounded-xl flex justify-between items-center mb-3">
                        <div class="flex gap-2 items-center">
                            ...
                            <div>
                                <h4 class="text-base font-semibold text-[#080808B2]">Get Bonus</h4>
        ...
        alert("Please add valid coupon code to get bonus!");
        return;
    }
});