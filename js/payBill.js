/*-------- Pay Bill toggler button -------*/
document.getElementById("pay-bill-btn").addEventListener("click", function () {
    //change btn click style
    toggleStyleBtn("pay-bill-btn");

    //show transfer money form section
    showFormSection("pay-bill-form");
});
...
document.getElementById("payBill-btn").addEventListener("click", function(event){
    ...
    const payAmount = getInputFieldValueById("pay-bill-amount");
    const pinNumber = getInputFieldValueById("payBill-pin");
    ...
    if(pinNumber === 1234){
        ...
        if(payAmount > currentBalance){
            alert("You have not enough balance to pay bill.");
            return;
        }
        ...
        div.innerHTML = `
            <div class="px-4 py-3 bg-white border border-[#0808081A] rounded-xl flex justify-between items-center mb-3">
                        <div class="flex gap-2 items-center">
                            ...
                            <div>
                                <h4 class="text-base font-semibold text-[#080808B2]">${reasonInput}</h4>
        ...
        alert("Failed to pay bill. Please try again later");
        return;
    }
});